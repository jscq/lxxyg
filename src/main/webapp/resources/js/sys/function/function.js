...
function searchFunction(){
	var queryParams = {
		'function.functionName' : $("#functionName").val(),
		'function.functionCode' : $('#functionCode').val(),
...
function save(formId,parameterName,token)
{
	...
	if (validateSubmit(formId)) {
			// 功能名称校验 
			var checkName = verifyFunctionName(parameterName,token);
...
//验证功能名称唯一性  
function verifyFunctionName(parameterName,token)
{
	var checkFlag = false;
	...
	var parentId = "";
	//隐藏功能名称
	var hiddenFunctionName = $("#hiddenFunctionName").val();
	var functionName = $("#functionName").val();
	functionName = functionName.replace(/(^\s*)|(\s*$)/g, "");
	if ((hiddenFunctionName=='')||(hiddenFunctionName != '' && hiddenFunctionName != functionName)) 
	{
		if(hiddenFunctionName=='' || hiddenFunctionName== undefined){
			parentId = $("#addParentId").val()
		}else{
			parentId = $("#editParentId").val()
		}
		checkFlag = true;
	}else if(hiddenFunctionName != '' && hiddenFunctionName == functionName)
	{
		return true;
	}
	...
	if(checkFlag)
	{
		if(functionName!='')
		{
			...
			$.ajax({
				...
				data : {
					"sysQuery.functionName" : functionName,
